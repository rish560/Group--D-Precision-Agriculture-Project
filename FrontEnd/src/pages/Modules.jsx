      ...
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="text-2xl font-semibold text-slate-900 dark:text-gray-100">Precision agriculture capabilities</h2>
          </div>
          <div className="rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 p-3 text-emerald-700"><Radar className="h-5 w-5" /></div>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {moduleCards.map(({ title, icon: Icon, description }) => (
            <div key={title} className="rounded-[1.5rem] border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 text-sm text-slate-700 dark:text-gray-300">
              <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700">
                <Icon className="h-5 w-5" />
              </div>
              <p className="font-semibold text-slate-900 dark:text-gray-100">{title}</p>
              <p className="mt-2 text-slate-600 dark:text-gray-400">{description}</p>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-3 text-sm text-slate-700 dark:text-gray-300">Revenue trend: {analytics.revenue.at(-1)}% above forecast</div>
            <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-3 text-sm text-slate-700 dark:text-gray-300">Water usage: {analytics.waterUsage.at(-1)}% of target</div>
            <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-3 text-sm text-slate-700 dark:text-gray-300">Farm performance: {analytics.farmPerformance.at(-1)}%</div>
          </div>
        </Card>
        <Card className="space-y-4">
