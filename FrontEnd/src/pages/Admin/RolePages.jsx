    ...
    <Card className="space-y-4">
      ...
      <div className="space-y-3">
        {users.filter((user) => !roleFilter || user.role === roleFilter).map((user) => (
          <div key={user.id} className="rounded-[1.5rem] border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 text-sm text-slate-700 dark:text-gray-300">
  ...
  return (
    <Card className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {farms.map((farm) => (
          <div key={farm.id} className="rounded-[1.5rem] border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 text-sm text-slate-700 dark:text-gray-300">
            <p className="font-semibold text-slate-900 dark:text-gray-100">{farm.name}</p>
...
export const AdminReportsPage = () => (
  <Card className="space-y-4">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <div className="rounded-[1.5rem] border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 text-sm text-slate-700 dark:text-gray-300">
