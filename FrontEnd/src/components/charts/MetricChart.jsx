...
export const MetricChart = ({ type = 'line', data = [], dataKey = 'value', color = '#10b981' }) => {
  if (!data.length) {
    return (
      <div className="flex h-56 w-full items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-500 dark:text-gray-400">
  ...
  if (type === 'bar') {
    return (
      <div className="h-56 w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 p-4 shadow-sm">
            ...
            return (
              <div key={`${item.name}-${index}`} className="flex flex-1 flex-col items-center gap-2">
                ...
                <div className="text-center text-xs text-slate-500 dark:text-gray-400">
                  <div className="font-medium text-slate-700 dark:text-gray-300">{item.name}</div>
                  <div>{value}</div>
                </div>
              </div>
            );
  ...
  if (type === 'pie') {
    ...
    return (
      <div className="flex h-56 w-full items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/70 p-4 shadow-sm">
        ...
        <div className="ml-4 space-y-2 text-sm">
          {data.map((item, index) => (
            <div key={`${item.name}-${index}`} className="flex items-center gap-2">
              ...
              />
              <span className="text-slate-600 dark:text-gray-400">{item.name}</span>
            </div>
          ))}
        </div>
      </div>
